...
int main(int argc, char* argv[])
{
    ...
    if (3 != argc)
    {
        char input[128]  = "bevel.obj";
        char output[128] = "model.hex";
    ...
    while (fgets(buffer, sizeof(buffer), pFileInput))
    {
        if ('v' == buffer[0])
        {
            ...
            else if ('n' == buffer[1]) { /* process normal direction */ }
        }
        else if ('f' == buffer[0])
        {
            /* process index information */
            sscanf(buffer, "f %d/%d/%d %d/%d/%d %d/%d/%d", &indices[nIndexes].v, &indices[nIndexes].t, &indices[nIndexes].n, &indices[nIndexes + 1].v, &indices[nIndexes + 1].t,
                &indices[nIndexes + 1].n, &indices[nIndexes + 2].v, &indices[nIndexes + 2].t, &indices[nIndexes + 2].n);
    ...
    for (uint32_t idx = 0U; idx < header.nIndices; ++idx)
    {
        ...
        pOutputIndexs[idx] = map[tempIndex->v][tempIndex->t];
    }
