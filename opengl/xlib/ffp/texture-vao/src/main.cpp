...
struct Vertex* pVertices = NULL; // vertex data
int*           pIndices  = NULL;
...
int main()
{
    ...
    pVertices = (struct Vertex*)malloc(sizeof(struct Vertex) * header.nVertices);
    pIndices  = (int*)malloc(sizeof(int) * header.nIndices);
    fread(pVertices, sizeof(struct Vertex), header.nVertices, pFile);
    fread(pIndices, sizeof(int), header.nIndices, pFile);
    // for (uint32_t idx = 0; idx < header.nVertices; ++idx) { printf("[%f %f %f : %f %f]\n", pVertices[idx].x, pVertices[idx].y, pVertices[idx].z, pVertices[idx].u, pVertices[idx].v); }
    // for (uint32_t idx = 0; idx < header.nIndices; ++idx) { printf("Index %d: %d\n", idx, pIndices[idx]); }
    fclose(pFile);
...
void display()
{
    ...
    struct Vertex* tempVertex = NULL;
    for (uint32_t idx = 0U; idx < header.nIndices; ++idx)
    {
        tempVertex = &pVertices[pIndices[idx]];
        glTexCoord2f(tempVertex->u, tempVertex->v);
