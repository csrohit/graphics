...
Display*     dpy         = nullptrptr; // connection to server
Colormap     colormap    = 0UL;
Window       window      = 0UL;     // handle of current window
XVisualInfo* pVisualInfo = nullptrptr; // Pointer to current visual info
bool         gbAbortFlag = false;   // Global abort flag
GLXContext   glxContext  = nullptrptr; // handle to OpenGL context
...
int initialize()
{
    ...
    glxContext   = glXCreateContext(dpy, pVisualInfo, nullptrptr, GL_TRUE);
    ...
    glVertexAttribPointer(AMC_ATTRIBUTE_POSITION, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), nullptrptr);
...
void uninitialize()
{
    ...
    if (0U != shaderProgramObject)
    {
        ...
        if (0 < nShaders)
        {
            ...
            if (nullptrptr != pShaders)
            {
                glGetAttachedShaders(shaderProgramObject, nShaders, nullptrptr, pShaders);
...
GLint compileShader(unsigned int shaderId, const char* shaderSourceCode)
{
    ...
    glShaderSource(shaderId, 1, (const GLchar**)&shaderSourceCode, nullptrptr);
    ...
    if (GL_FALSE == status)
    {
        ...
        if (0 != infoLogLength)
        {
            ...
            if (nullptrptr != szInfoLog)
            {
                glGetShaderInfoLog(shaderId, infoLogLength, nullptrptr, szInfoLog);
                fprintf(gpFile, "Shader compilation error log: %s\n", szInfoLog);
                free(szInfoLog);
                szInfoLog = nullptrptr;
            }
        }
    }
    return status;
...
GLint linkProgram(GLuint programId)
{
    ...
    if (GL_FALSE == status)
    {
        ...
        if (0 != infoLogLength)
        {
            ...
            if (nullptrptr != szInfoLog)
            {
                glGetProgramInfoLog(programId, infoLogLength, nullptrptr, szInfoLog);
